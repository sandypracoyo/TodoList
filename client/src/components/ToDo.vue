<template>
  <div class="hello">

    <div id="todo-list-example" class="container">
      <div class="row">
        <div class="col-md-6 mx-auto">
          <h1 class="text-center">TODO List App</h1>
          <form v-on:submit.prevent="addNewTask">
            <label for="tasknameinput">Task Name</label>
            <input v-model="taskname" id="tasknameinput" class="form-control" placeholder="Add New Task">
            <button v-if="this.done == false" type="submit" class="btn btn-success btn-block  mt-3">
              Submit
            </button>
            <button v-else type="button" v-on:click="updateTask()" class="btn btn-primary btn-block  mt-3">
              Update
            </button>
          </form>

          <table class="table">
            <tr v-for="(todo) in todos" v-bind:key="todo.id" v-bind:title="todo.nama_task">
              <td class="text-left">{{todo.nama_task}}</td>
              <td class="text-right">
                <button v-on:click="editTask(todo.nama_task, todo.id)" class=" btn btn-info ">Edit</button>
                <button v-on:click="deleteTask(todo.id)" class=" btn btn-danger ">Delete</button>
              </td>
            </tr>
          </table>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import axios from 'axios'
export default {
  data () {
    return {
      todos: [],
      id: '',
      nama_task: '',
      done: false
    }
  },
  mounted () {
    this.getTasks()
  },
  methods: {
    getTasks () {
      axios.get('http://localhost:5000/api').then(
        result => {
          console.log(result.data.data)
          this.todos = result.data.data
        },
        error => {
          console.error(error)
        }
      )
    },
    addNewTask () {
      axios.post('http://localhost:5000/api',
        { nama_task: this.taskname }
      ).then((res) => {
        this.taskname = ''
        this.getTasks()
        console.log(res)
      }).catch((err) => {
        console.log(err)
      })
    },
    editTask (title, id) {
      this.id = id
      this.taskname = title
      this.done = true
    },
    updateTask () {
      axios.put(`http://localhost:5000/api/${this.id}`,
        { nama_task: this.taskname }
      ).then((res) => {
        this.taskname = ''
        this.done = false
        this.getTasks()
        console.log(res)
      }).catch((err) => {
        console.log(err)
      })
    },
    deleteTask (id) {
      axios.delete(`http://localhost:5000/api/${id}`
      ).then((res) => {
        this.taskname = ''
        this.getTasks()
        console.log(res)
      }).catch((err) => {
        console.log(err)
      })
    }
  }
}
</script>
